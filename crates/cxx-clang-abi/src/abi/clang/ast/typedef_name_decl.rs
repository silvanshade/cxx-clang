...
#[cxx::bridge]
mod ffi {
    #[namespace = "cxx_clang::clang::ast::typedef_name_decl"]
    unsafe extern "C++" {
        include!("cxx-clang-abi/cxx/include/clang/AST/TypedefNameDecl.hxx");
...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["clang", "ast", "typedef_name_decl"];
    let path_descendants = vec![];
    let cxx_include = "cxx-clang-abi/cxx/include/clang/AST/TypedefNameDecl.hxx";
    let cxx_namespace = "cxx_clang::clang::ast::typedef_name_decl";
    let cxx_name = "TypedefNameDecl";
    let rust_name = "TypedefNameDecl";
