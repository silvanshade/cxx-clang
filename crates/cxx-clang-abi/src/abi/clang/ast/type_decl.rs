...
#[cxx::bridge]
mod ffi {
    #[namespace = "cxx_clang::clang::ast::type_decl"]
    unsafe extern "C++" {
        include!("cxx-clang-abi/cxx/include/clang/AST/TypeDecl.hxx");
...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["clang", "ast", "type_decl"];
    let path_descendants = vec![];
    let cxx_include = "cxx-clang-abi/cxx/include/clang/AST/TypeDecl.hxx";
    let cxx_namespace = "cxx_clang::clang::ast::type_decl";
    let cxx_name = "TypeDecl";
    let rust_name = "TypeDecl";
