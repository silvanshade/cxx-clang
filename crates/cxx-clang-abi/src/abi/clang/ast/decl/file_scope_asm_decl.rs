...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["clang", "ast", "decl", "file_scope_asm_decl"];
    let path_descendants = vec![];
    let cxx_include = "cxx-clang-abi/cxx/include/clang/AST/Decl/FileScopeAsmDecl.hxx";
    let cxx_namespace = "cxx_clang::clang::ast::decl::file_scope_asm_decl";
    let cxx_name = "FileScopeAsmDecl";
    let rust_name = "FileScopeAsmDecl";
