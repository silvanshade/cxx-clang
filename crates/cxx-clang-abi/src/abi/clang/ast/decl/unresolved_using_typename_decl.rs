...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["clang", "ast", "decl", "unresolved_using_typename_decl"];
    let path_descendants = vec![];
    let cxx_include = "cxx-clang-abi/cxx/include/clang/AST/Decl/UnresolvedUsingTypenameDecl.hxx";
    let cxx_namespace = "cxx_clang::clang::ast::decl::unresolved_using_typename_decl";
    let cxx_name = "UnresolvedUsingTypenameDecl";
    let rust_name = "UnresolvedUsingTypenameDecl";
