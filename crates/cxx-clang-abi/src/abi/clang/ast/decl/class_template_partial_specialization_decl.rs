...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["clang", "ast", "decl", "class_template_partial_specialization_decl"];
    let path_descendants = vec![];
    let cxx_include = "cxx-clang-abi/cxx/include/clang/AST/Decl/ClassTemplatePartialSpecializationDecl.hxx";
    let cxx_namespace = "cxx_clang::clang::ast::decl::class_template_partial_specialization_decl";
    let cxx_name = "ClassTemplatePartialSpecializationDecl";
    let rust_name = "ClassTemplatePartialSpecializationDecl";
