...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["clang", "ast", "decl", "ms_guid_decl"];
    let path_descendants = vec![];
    let cxx_include = "cxx-clang-abi/cxx/include/clang/AST/Decl/MSGuidDecl.hxx";
    let cxx_namespace = "cxx_clang::clang::ast::decl::ms_guid_decl";
    let cxx_name = "MSGuidDecl";
    let rust_name = "MsGuidDecl";
