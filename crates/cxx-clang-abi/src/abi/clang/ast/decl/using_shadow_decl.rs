...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["clang", "ast", "decl", "using_shadow_decl"];
    let path_descendants = vec![];
    let cxx_include = "cxx-clang-abi/cxx/include/clang/AST/Decl/UsingShadowDecl.hxx";
    let cxx_namespace = "cxx_clang::clang::ast::decl::using_shadow_decl";
    let cxx_name = "UsingShadowDecl";
    let rust_name = "UsingShadowDecl";
