...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["clang", "ast", "decl", "empty_decl"];
    let path_descendants = vec![];
    let cxx_include = "cxx-clang-abi/cxx/include/clang/AST/Decl/EmptyDecl.hxx";
    let cxx_namespace = "cxx_clang::clang::ast::decl::empty_decl";
    let cxx_name = "EmptyDecl";
    let rust_name = "EmptyDecl";
