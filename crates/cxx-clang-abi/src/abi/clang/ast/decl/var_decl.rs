...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["clang", "ast", "decl", "var_decl"];
    let path_descendants = vec![];
    let cxx_include = "cxx-clang-abi/cxx/include/clang/AST/Decl/VarDecl.hxx";
    let cxx_namespace = "cxx_clang::clang::ast::decl::var_decl";
    let cxx_name = "VarDecl";
    let rust_name = "VarDecl";
