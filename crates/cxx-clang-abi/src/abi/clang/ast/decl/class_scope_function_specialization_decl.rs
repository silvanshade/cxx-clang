...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["clang", "ast", "decl", "class_scope_function_specialization_decl"];
    let path_descendants = vec![];
    let cxx_include = "cxx-clang-abi/cxx/include/clang/AST/Decl/ClassScopeFunctionSpecializationDecl.hxx";
    let cxx_namespace = "cxx_clang::clang::ast::decl::class_scope_function_specialization_decl";
    let cxx_name = "ClassScopeFunctionSpecializationDecl";
    let rust_name = "ClassScopeFunctionSpecializationDecl";
