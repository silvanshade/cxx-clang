...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["clang", "ast", "decl", "decl_context"];
    let path_descendants = vec![];
    let cxx_include = "cxx-clang-abi/cxx/include/clang/AST/Decl/DeclContext.hxx";
    let cxx_namespace = "cxx_clang::clang::ast::decl::decl_context";
    let cxx_name = "DeclContext";
    let rust_name = "DeclContext";
