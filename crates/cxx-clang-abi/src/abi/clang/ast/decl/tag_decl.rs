...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["clang", "ast", "decl", "tag_decl"];
    let path_descendants = vec![];
    let cxx_include = "cxx-clang-abi/cxx/include/clang/AST/Decl/TagDecl.hxx";
    let cxx_namespace = "cxx_clang::clang::ast::decl::tag_decl";
    let cxx_name = "TagDecl";
    let rust_name = "TagDecl";
