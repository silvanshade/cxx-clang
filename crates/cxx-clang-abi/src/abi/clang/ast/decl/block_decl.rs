...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["clang", "ast", "decl", "block_decl"];
    let path_descendants = vec![];
    let cxx_include = "cxx-clang-abi/cxx/include/clang/AST/Decl/BlockDecl.hxx";
    let cxx_namespace = "cxx_clang::clang::ast::decl::block_decl";
    let cxx_name = "BlockDecl";
    let rust_name = "BlockDecl";
