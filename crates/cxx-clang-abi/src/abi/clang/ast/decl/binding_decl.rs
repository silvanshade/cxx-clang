...
fn artifact_info() -> ::cxx_memory_abi::CxxAbiArtifactInfo {
    let path_components = vec!["clang", "ast", "decl", "binding_decl"];
    let path_descendants = vec![];
    let cxx_include = "cxx-clang-abi/cxx/include/clang/AST/Decl/BindingDecl.hxx";
    let cxx_namespace = "cxx_clang::clang::ast::decl::binding_decl";
    let cxx_name = "BindingDecl";
    let rust_name = "BindingDecl";
