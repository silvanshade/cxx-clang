use crate::{
    ...
    gen::clang::ast::decl::obj_c_method_decl,
};
use core::pin::Pin;

pub use crate::auto::clang::ast::decl::obj_c_method_decl::ObjCMethodDecl;

impl<'ctx> ObjCMethodDecl<'ctx> {
    #[inline]
    pub fn as_ref_decl_context(&self) -> &DeclContext<'ctx> {
        obj_c_method_decl::as_ref_decl_context(self)
    }

    #[inline]
    pub fn as_pin_decl_context(self: Pin<&mut Self>) -> Pin<&mut DeclContext<'ctx>> {
        obj_c_method_decl::as_pin_decl_context(self)
    }

    #[inline]
    pub fn as_ref_named_decl(&self) -> &NamedDecl<'ctx> {
        obj_c_method_decl::as_ref_named_decl(self)
    }

    #[inline]
    pub fn as_pin_named_decl(self: Pin<&mut Self>) -> Pin<&mut NamedDecl<'ctx>> {
        obj_c_method_decl::as_pin_named_decl(self)
    }
}
